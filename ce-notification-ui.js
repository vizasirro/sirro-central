(() => {
  ...
  function enhanceCard(card){
    ...
    card.innerHTML=`<div class="sirro-ce-head"><strong>${isReprogram?'CITA REPROGRAMADA':'CITA PROGRAMADA'} – CONSULTA EXTERNA</strong></div>
      ...
      <div class="sirro-ce-instruction">Informar al paciente que debe presentarse en la fecha y hora indicadas.</div>`;
    const actions=document.createElement('div');actions.className='actions';const btn=document.createElement('button');btn.type='button';btn.textContent='VER CITA';btn.onclick=()=>{const tab=document.querySelector('#tabs button[data-tab="notificaciones"]');if(tab)tab.click();};actions.appendChild(btn);card.appendChild(actions);
  }
