(() => {
  ...
  const coloredBox=(kind,html)=>{const c=colors[kind];return `<div style="background:${c.bg};border:2px solid ${c.border};color:${c.text};padding:12px;border-radius:12px">${html}</div>`;};
  ...
  function renderHomePending(){
    ...
    box.innerHTML=`<div style="display:flex;gap:8px;flex-wrap:wrap">${coloredBox('pending',`<strong>${red}</strong> PENDIENTE${red===1?'':'S'}`)}${coloredBox('attention',`<strong>${yellow}</strong> REQUIERE${yellow===1?'':'N'} MI ATENCIÓN`)}${other?coloredBox('upcoming',`<strong>${other}</strong> PRÓXIMO${other===1?'':'S'} / AVISO${other===1?'':'S'}`):''}</div>`;
  }
