(() => {
  ...
  const coloredButton=(kind,html)=>{const c=colors[kind];return `<button type="button" data-pending-group="${kind}" style="background:${c.bg};border:2px solid ${c.border};color:${c.text};padding:12px 16px;border-radius:12px;font:inherit;cursor:pointer;text-align:left;touch-action:manipulation">${html}</button>`;};
  ...
  window.openSirroPending=openPending;
  if(!window.__sirroPendingDelegatedClick){
    window.__sirroPendingDelegatedClick=true;
    document.addEventListener('click',e=>{
      const trigger=e.target.closest?.('#sirroPendingOpen,[data-pending-group]');
      if(!trigger)return;
      e.preventDefault();
      e.stopPropagation();
      openPending(trigger.dataset.pendingGroup||null);
    },true);
  }
