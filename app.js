...
async function init(){
 if(!cfg.SUPABASE_URL||!cfg.SUPABASE_ANON_KEY||cfg.SUPABASE_ANON_KEY.includes('PEGAR_AQUI')){
   msg('#loginMsg','Falta colocar la clave pública de Supabase en config.js.',true);return;
 }
...
async function login(){
 ...
 if(error)return msg('#loginMsg','Usuario o contraseña incorrectos.',true);
...
async function enter(user){
 ...
 profile=data;
 if(profile.estado!=='ACTIVO'){await sb.auth.signOut();return msg('#loginMsg','Usuario no activo: '+profile.estado,true)}
...
function configureTabs(){
 ...
 $('#originBox').innerHTML=o?`<strong>Origen:</strong> ${esc(o.nombre)} · RUPS ${esc(o.codigo_rups)}`:'';
 fillDestinations();
}
function fillDestinations(){
 const sel=$('#destFacility'); if(!sel)return;
 let list=establishments.filter(x=>x.id!==profile.establecimiento_id);
...
async function stats(){
 const a=await sb.from('establecimientos').select('*',{count:'exact',head:true});
 const c=await sb.from('casos_referencia').select('estado_actual');
 const rows=c.data||[];$('#statEst').textContent=a.count??'—';$('#statCasos').textContent=rows.length;
 $('#statAbiertos').textContent=rows.filter(x=>['ABIERTO','EN_PROCESO'].includes(x.estado_actual)).length;
 $('#statCerrados').textContent=rows.filter(x=>String(x.estado_actual).startsWith('CERRADO')).length;
}
async function refs(){
 ...
 const box=$('#refsList');if(error){box.innerHTML='<p>No se pudieron cargar las referencias.</p>';return}
...
async function users(){
 if(profile?.rol!=='ADMIN_REGIONAL')return;
 const {data}=await sb.from('perfiles').select('nombre_completo,rol,estado,correo,telefono').order('nombre_completo');
 $('#usersList').innerHTML=(data||[]).map(u=>`<div class="item"><strong>${esc(u.nombre_completo)}</strong><br><small>${esc(u.rol)} · ${esc(u.estado)} · ${esc(u.correo)} · ${esc(u.telefono)}</small></div>`).join('');
}
async function audit(){
 if(!['ADMIN_REGIONAL','AUDITOR_CONSULTA'].includes(profile?.rol))return;
 const {data}=await sb.from('auditoria').select('accion,tabla,registro_id,motivo,creado_en').order('creado_en',{ascending:false}).limit(100);
 $('#auditList').innerHTML=(data||[]).map(a=>`<div class="item"><strong>${esc(a.accion)}</strong> · ${esc(a.tabla)}<br><small>${fmt(a.creado_en)} ${a.motivo?'· '+esc(a.motivo):''}</small></div>`).join('');
}
async function createRef(e){
 e.preventDefault();msg('#refMsg','Registrando…');
 if(!profile.establecimiento_id)return msg('#refMsg','Su perfil no tiene establecimiento asignado.',true);
 const dest=$('#destFacility').value;if(!dest)return msg('#refMsg','Seleccione destino.',true);
 // El código visible definitivo y el tramo deben generarse en servidor para evitar correlativos duplicados.
 // Esta interfaz bloquea el alta si la RPC de creación central aún no existe.
 ...
 const {data,error}=await sb.rpc('crear_referencia_sirro',payload);
 if(error)return msg('#refMsg','La base central aún necesita habilitar la función crear_referencia_sirro. No se guardó ningún dato.',true);
