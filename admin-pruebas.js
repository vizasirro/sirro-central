(() => {
  ...
  function updateTestModeCopy(){
    ...
    if(card){
      ...
      if(resetBtn)resetBtn.textContent='Borrar referencias y usuarios de prueba';
    }
  }
  ...
  async function deleteTestUser(id){
    ...
    const {data,error}=await sb.functions.invoke('sirro-delete-user',{body:{user_id:id,confirmacion:'BORRAR USUARIO'}});
    if(error)return alert(error.message||'No se pudo borrar el usuario.');
