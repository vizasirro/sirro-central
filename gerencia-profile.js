(() => {
  ...
  const allEcors=()=>typeof ecors!=='undefined'&&Array.isArray(ecors)?ecors:[];
  const allMunicipios=()=>typeof municipios!=='undefined'&&Array.isArray(municipios)?municipios:[];
  const allEstablishments=()=>typeof establishments!=='undefined'&&Array.isArray(establishments)?establishments:[];
  ...
  function addGerenciaControls(){
    ...
    if(![...role.options].some(o=>o.value==='GERENCIA')){const opt=document.createElement('option');opt.value='GERENCIA';opt.textContent='GERENCIA';role.appendChild(opt);}
    ...
    function fillEntity(){
      ...
      if(v==='ECOR'){entityLabel.textContent='ECOR';entity.innerHTML+=allEcors().map(x=>`<option value="${x.id}">${esc(x.nombre)}</option>`).join('');}
      if(v==='MUNICIPIO'){entityLabel.textContent='Municipio';entity.innerHTML+=allMunicipios().map(x=>`<option value="${x.id}">${esc(x.nombre)}</option>`).join('');}
      if(v==='HOSPITAL'){entityLabel.textContent='Hospital';entity.innerHTML+=allEstablishments().filter(x=>x.tipo==='HOSPITAL'&&!x.es_externo_olancho).map(x=>`<option value="${x.id}">${esc(x.nombre)}</option>`).join('');}
    ...
    function updateUI(){
      ...
      ['newScopeWrap','newMonitorWrap','newEcorWrap','newMunicipioWrap','newEstWrap','newHospitalUserTypeWrap','newHospitalSpecialtyWrap'].forEach(id=>document.getElementById(id)?.classList.add('hidden'));
    }
    ...
    form.addEventListener('submit',e=>{
      ...
      hiddenMonitor.value='true';hiddenEcor.value='';hiddenMun.value='';hiddenEst.value='';
      ...
      if(v==='HOSPITAL'){
        hiddenScope.value='ESTABLECIMIENTO';const h=allEstablishments().find(x=>String(x.id)===String(entity.value));
        if(!h){e.preventDefault();e.stopImmediatePropagation();showMsg('#userMsg','Seleccione un hospital válido.','error');return;}
        hiddenMun.value=h.municipio_id||'';if(typeof fillUserEstablishments==='function')fillUserEstablishments();hiddenEst.value=h.id;
      }
  ...
  function applyGerenciaIdentity(){
    ...
    const meta=document.getElementById('userMeta');if(meta)meta.textContent=`GERENCIA · ${gerenciaLevel(profile)}`;
  }
